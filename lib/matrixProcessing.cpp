 ...
 *   - detLU: determinant of a matrix LU decomposition
 *   - deteig: determinant of a matrix eigenvalue decomposition
 ...
 */
Matrix getType(Matrix &inmtx, int t)
{
 ...
 */
Matrix subMatrix(Matrix &inmtx, int srow, int scol, int erow, int ecol) {
  int i, j;
  int nr, nc;
  Matrix temp;
 ...
 */
Matrix mean(Matrix &inmtx, int nf)
{
  Matrix temp;
  int nr, nc;
  int i, j;
 ...
 */
Matrix cov(Matrix &inmtx, int nf)
{    
  Matrix mu, temp, x, y;
  int nr, nc;
 ...
 */
Matrix ludcmp(Matrix &src, double& d)
{
  int n = src.getRow();
  if (n != src.getCol()) {
 ...
 */
void insertsort(Matrix &inmtx, Matrix &sorted, Matrix &pos)
{
  double inserter;
  int i, l, index;
  int nr, nc;
